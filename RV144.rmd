...
```{r}
rv144 <- chisqtestGC(results, graph = TRUE)
rv144
```
...
As a second approarch we will consider the fisher exact test. We will state the null and 

$H_0:$ The odds ratio between having aids and getting the vaccine contains thye number 1.
  
$H_a:$ The odds ratio between getting aids andf having the vaccine contains does not contain the number 1.

```{r}
myfish<- fisher.test(results)
myfish
```
The p value from the fisher test is 'r  myfish$p.value' and the confidence interval is 'r  myfish$condf.int'
